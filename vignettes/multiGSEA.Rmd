...
# TODO: 
# 1. update text to favor `enrichtest`
# 2. describe how its data.frame input works (enrichtest)
...
```{r multiGSEA-methods, results='asis', echo=FALSE, message=FALSE, warning=FALSE}
...
mg <- multiGSEA(
  ...
  methods = c('camera', 'fry', 'enrichtest'),
...
cam.go.res <- results(mg, c('camera', 'enrichtest.up'))
```
...
mgheatmap(vm.bh, gdb.2, aggregate.by = "none", split = TRUE,
          ...
          top_annotation=col.anno, zlim = c(-3, 3))
```
...
[GeneSetDb.MSigDB](https://github.com/lianos/GeneSetDb.MSigDB) package.
