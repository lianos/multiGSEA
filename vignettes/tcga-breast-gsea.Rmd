...
Alternatively, this document defines `knit: rmd.plugins::report.render` in the
...
The results of a call to `multiGSEA` can be interrogated at the command line.
Some functions that are helpful here include:
...
Please refer to their help documentation for more information.
...
`MultiGSEAResult` into a browseable widget for an rmarkdown document. It provides
several parameters that can be used to customize which (and how many) results
are shown.
...
rmd.multiGSEA('tumor-vs-normal-gsea-top10-fdr', mg, plot.stat='t', top.n=10,
              max.p=0.30)
```
