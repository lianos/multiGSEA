...
gsea.methods <- c('camera', 'geneSetTest', 'enrichtest', 'roast')
...
gsd.all <- getMSigGeneSetDb(c('h', 'c2', 'c7'), 'human', 'entrez')
...
gsd <- append(getMSigGeneSetDb('h', id.type = 'entrez'), exampleGeneSetDb())
...
hypergeometric approach. `multiGSEA` provides the `enrichtest`method, which
is a wrappter to `limma::kegga` that works with a `GeneSetDb` object. The test
can optionally adjust for dge detection by controlling for a numeric covariate,
such as gene length (like "goseq" does).
...
# Run an unbiased hypergeometric test
enriced <- enrichtest(gsd, lfc, selected = "significant")
```

These are the pathways with an FDR <= 0.3:

`r knitr::kable(subset(enriched, P.all < 0.1)`
